...
type SeasonQuery struct {
	...
	withScores         *SeasonScoreQuery
	withTaskProgresses *SeasonTaskProgressQuery
...
func (_q *SeasonQuery) QueryScores() *SeasonScoreQuery {
...
// QueryTaskProgresses chains the current query on the "task_progresses" edge.
func (_q *SeasonQuery) QueryTaskProgresses() *SeasonTaskProgressQuery {
...
func (_q *SeasonQuery) Clone() *SeasonQuery {
	...
	return &SeasonQuery{
		...
		withTaskProgresses: _q.withTaskProgresses.Clone(),
...
func (_q *SeasonQuery) WithScores(opts ...func(*SeasonScoreQuery)) *SeasonQuery {
	query := (&SeasonScoreClient{config: _q.config}).Query()
...
// WithTaskProgresses tells the query-builder to eager-load the nodes that are connected to
// the "task_progresses" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SeasonQuery) WithTaskProgresses(opts ...func(*SeasonTaskProgressQuery)) *SeasonQuery {
	query := (&SeasonTaskProgressClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTaskProgresses = query
	return _q
}
...
func (_q *SeasonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Season, error) {
	...
	if query := _q.withScores; query != nil {
		if err := _q.loadScores(ctx, query, nodes,
			func(n *Season) { n.Edges.Scores = []*SeasonScore{} },
			func(n *Season, e *SeasonScore) { n.Edges.Scores = append(n.Edges.Scores, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTaskProgresses; query != nil {
		if err := _q.loadTaskProgresses(ctx, query, nodes,
			func(n *Season) { n.Edges.TaskProgresses = []*SeasonTaskProgress{} },
			func(n *Season, e *SeasonTaskProgress) { n.Edges.TaskProgresses = append(n.Edges.TaskProgresses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SeasonQuery) loadScores(ctx context.Context, query *SeasonScoreQuery, nodes []*Season, init func(*Season), assign func(*Season, *SeasonScore)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seasonscore.FieldSeasonID)
	}
	query.Where(predicate.SeasonScore(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(season.ScoresColumn), fks...))
	}))
...
func (_q *SeasonQuery) loadTaskProgresses(ctx context.Context, query *SeasonTaskProgressQuery, nodes []*Season, init func(*Season), assign func(*Season, *SeasonTaskProgress)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Season)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seasontaskprogress.FieldSeasonID)
	}
	query.Where(predicate.SeasonTaskProgress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(season.TaskProgressesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
